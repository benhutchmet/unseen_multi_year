...
def plot_multi_var_perc(
    ...
    if y1_hlines is not None:
        ...
        # Set y1_hlines as the min and max of y1 values
        y1_hlines = [
            np.min(model_percs_5[f"{y_var_name_model}_mean"]),
            np.max(model_percs_5[f"{y_var_name_model}_mean"])
        ]
    ...
    if y2_hlines is not None:
        ...
        y2_hlines = [
            np.min(model_percs_5[f"{y2_var_name_model}_mean"]),
            np.max(model_percs_5[f"{y2_var_name_model}_mean"])
        ]
        ...
        for y2_hline in y2_hlines:
            ...
            # Add text label above the line
            ax2.text(
                0.6,  # x position (60% from left edge, on the right side)
...
def main():
    ...
    # extract the unique leads from full_wp_gen_df
    unique_leads = full_wp_gen_df["lead"].unique()

    # Print the min and max unique leads
    print(f"Min lead: {unique_leads.min()}")
    print(f"Max lead: {unique_leads.max()}")
    ...
    # print the head of full_wp_gen_df
    print(full_wp_gen_df.head())

    # print the tail of full_wp_gen_df
    print(full_wp_gen_df.tail())
    ...
    # Reset the index of the wp generation dataframe
    # df_obs_wp_generation.reset_index(inplace=True)
    ...
    # # Subset the df_model_djf_new to this init_year, member, and winter_year
    # df_model_djf_new_test = df_model_djf_new[
    #     (df_model_djf_new["init_year"] == test_init_year)
    #     & (df_model_djf_new["member"] == test_member)
    #     & (df_model_djf_new["winter_year"] == test_winter_year)
    # ]
    ...
    # # Plot the model wind speed data
    # ax.plot(
    #     df_model_djf_new_test["lead"],
    #     df_model_djf_new_test["data_sfcWind"],
    #     color="black",
    #     label="Model Wind Speed (m/s)",
    # )
    ...
    # # Plot the model wind power generation data
    # ax2.plot(
    #     df_model_djf_new_test["lead"],  # Offset by 29 days to align with leads
    #     df_model_djf_new_test["wind_cfs"],
    #     color="red",
    #     label="Model CFs",
    # )
    ...
    # # Set the title
    # ax.set_title(
    #     f"Model Wind Speed and Wind Power Generation\n"
    #     f"Init Year: {test_init_year}, Member: {test_member}, "
    #     f"Winter Year: {test_winter_year}\n"
    #     f"Pearson Correlation: {corr:.2f}",
    #     fontsize=12,
    ...
    df_obs.rename(
        columns={"Wind Power Generation (GW)": "data_wp_generation"},
        inplace=True,
    )
    ...
    gev_funcs.dot_plot_subplots(
        ...
        title_left="c) Maximum demand net wind winter day (GW)",
